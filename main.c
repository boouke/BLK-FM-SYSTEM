...
void dir_create(void) {
    ...
    while (scanf("%s", dir_name) != 1) {
        printf("Invalid input. Please enter a valid directory name: ");
    }

    // Create the directory
    int creating = mkdir(dir_name);
    ...
    printf("Set permissions for others (1 = Read-Only, 2 = Read-Write): ");
    scanf("%d", &others_perms);

    int permission_mode = 0;

    // Convert user selections to permission bits
    if (others_perms == 2) {
        permission_mode |= S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | S_IWOTH;
    } else {
        permission_mode |= S_IRUSR | S_IRGRP | S_IROTH;
    }

    if (chmod(dir_name, permission_mode) != 0) {
